@extends('layouts.app')
@section('title', 'La mia collezione: ' . $set->name)
@section('meta_description', 'Dettaglio della tua collezione nel set ' . $set->name)
...
@section('content')
        ...
        <div class="container">
            <nav aria-label="breadcrumb" class="mb-4">
                <ol class="breadcrumb bg-transparent p-0 mb-0">
                    <li class="breadcrumb-item"><a class="text-decoration-none text-secondary"
                            href="{{ route('collezioni.mie') }}">Le mie Collezioni</a></li>
                    @if ($set->serie)
                        <li class="breadcrumb-item"><a class="text-decoration-none text-secondary"
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start gap-3 mb-4">
                <div>
                    ...
                    <p class="text-secondary mb-0">La tua collezione • {{ $userCards->total() }} carte salvate in questo set
                    </p>
                </div>
                    ...
                    <form id="filter-form" method="GET" action="{{ route('collezioni.mie.set', ['set' => $set->id]) }}">
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-6">
                                <label for="search" class="form-label text-secondary small text-uppercase">Nome
                                    carta</label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="type" class="form-label text-secondary small text-uppercase">Tipo
                                    Pokémon</label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="stage" class="form-label text-secondary small text-uppercase">Stadio
                                    evolutivo</label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="sort" class="form-label text-secondary small text-uppercase">Ordina
                                    per</label>
                            ...
                            <div class="col-12 col-md-6">
                                <label for="per_page" class="form-label text-secondary small text-uppercase">Carte per
                                    pagina</label>
            ...
            <div class="row gy-4">
                <div class="col-12 col-xl-8">
                    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4" id="cards-grid">
                        @include('collezioni.partials.my-cards-grid', ['userCards' => $userCards])
                    </div>
                ...
                <div class="col-12 col-xl-4">
                    <div class="card summary-card h-100">
                        <div class="card-body">
                            <h2 class="h5 text-white">Riepilogo collezione nel set</h2>
                            <p class="text-secondary">Informazioni sulla tua collezione per questo specifico set.</p>
                            <div class="list-group list-group-flush mt-4">
                                <div
                                    class="list-group-item bg-transparent px-0 py-2 d-flex justify-content-between align-items-center border-0">
                                    <span class="summary-label">Carte possedute</span>
                                ...
                                <div
                                    class="list-group-item bg-transparent px-0 py-2 d-flex justify-content-between align-items-center border-0">
                                    <span class="summary-label">Completamento</span>
                                    <span class="summary-value">{{ $progress }}%</span>
                                </div>
                                <div
                                    class="list-group-item bg-transparent px-0 py-2 d-flex justify-content-between align-items-center border-0">
                                    <span class="summary-label">Lingua set</span>
    ...
    <div id="mass-action-bar" class="fixed-bottom p-3 d-flex justify-content-center" style="pointer-events: none; z-index: 1040; transition: transform 0.3s ease; transform: translateY(100%);">
        <div class="card bg-danger text-white shadow-lg border-0" style="pointer-events: auto; max-width: 400px; border-radius: 2rem;">
            <div class="card-body d-flex align-items-center gap-3 py-2 px-4">
                <span class="fw-bold"><span id="mass-selected-count">0</span> carte selezionate</span>
                <button class="btn btn-light btn-sm text-danger fw-bold rounded-pill" onclick="massRemoveSelected()">Elimina</button>
                <button class="btn btn-sm text-white text-opacity-75" onclick="clearSelection()">Annulla</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        window.removeEntireCard = async function(cardId, btnEl) {
            if (!confirm('Vuoi rimuovere completamente questa carta dalla tua collezione?')) return;
        ...
        window.massRemoveSelected = async function() {
            if (!confirm(`Vuoi rimuovere ${selectedCards.size} carte dalla tua collezione?`)) return;
